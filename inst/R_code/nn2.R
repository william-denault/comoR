...
make_cebnm_expmix_fun <- function(X, max_num_classes=10){
  cebnm_expmix_nn <- function(x,s,g_init=FALSE,fix_g=TRUE, output){
    ...
    scales <- autoselect_scales_mix_exp(x, s, max_num_classes)
    num_classes <- length(scales)
    rates <- 1/scales
...
william_example2 <- function(){
  ...
  return(list(fit_custom = fit_custom, fit_factor=fit_factor))
}
